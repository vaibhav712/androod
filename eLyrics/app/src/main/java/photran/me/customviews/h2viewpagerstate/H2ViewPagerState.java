...
public class H2ViewPagerState extends View {
	/**
	...
	/* Kích thước chiều cao tối thiểu của H2ViewPagerState (mặc định 16dp) */
	public static float HEIGHT_SIZE;
	/*
	 * Kích thước chiều rộng(RECT) hay đường kính (CIRCLE) các item state của
	 * H2ViewPagerState (mặc định 10dp)
	 */
	public static float WIDTH_STATE_SIZE;
	/* Khoảng cách của các item state H2ViewPagerState (mặc định 5dp) */
	public static float SPACE;
	/*
	 * Kích thước chiều cao (RECT) các item state của H2ViewPagerState (mặc định
	 * 4dp)
	 */
	public static float HEIGHT_STATE_SIZE;

	/* H2ViewPagerState dạng hình chữ nhật */
	public static final int RECT = 1;
	/* H2ViewPagerState dạng hình tròn (mặc định) */
	...
	public H2ViewPagerState(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
		readFromAttribute(context, attrs, defStyle);
	}

	public H2ViewPagerState(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public H2ViewPagerState(Context context) {
		this(context, null, 0);
	}
	...
	private void readFromAttribute(Context context, AttributeSet attrs,
		...
		if (attrs != null) {
			TypedArray a = context.obtainStyledAttributes(attrs,
					R.styleable.H2ViewPagerState, defStyle, 0);
			mColorNormalState = a.getColor(
					R.styleable.H2ViewPagerState_colorNormalState,
					mColorNormalState);
			mColorState = a.getColor(R.styleable.H2ViewPagerState_colorState,
					mColorState);
			mPaintNormalState.setColor(mColorNormalState);
			mPaintState.setColor(mColorState);

			mSize = a.getInt(R.styleable.H2ViewPagerState_numberItem, 1);
			mWidthItem = a.getFloat(R.styleable.H2ViewPagerState_widthItem,
					mWidthItem);
			mHeightItem = a.getFloat(R.styleable.H2ViewPagerState_heightItem,
					mWidthItem);
			if (mWidthItem > 0 && mHeightItem > 0) {
				...
				if (mWidthItem == mHeightItem) {
					...
					mStype = H2ViewPagerState.CIRCLE;

				} else {
					this.mWidthItem = Math.min(mWidthItem, WIDTH_STATE_SIZE);
					this.mHeightItem = Math.min(mHeightItem, HEIGHT_STATE_SIZE);
					mStype = H2ViewPagerState.RECT;
				}
			} else {
	...
	protected void onDraw(Canvas canvas) {
		...
		case RECT:
			...
			break;
		// cần thiết cho những trường hợp khác
